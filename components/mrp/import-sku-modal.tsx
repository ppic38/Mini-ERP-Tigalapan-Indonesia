...
import { parseSkuImportFile, downloadSkuImportTemplate, type ParsedSkuRow } from "@/lib/mrp/parseSkuImport";
import { useMrpStore } from "@/lib/mrp/store";
import type { SkuImportSummary } from "@/lib/mrp/actions";

/** Popup "Import Data" untuk Master Data SKU (owner 2026-09-24). Alur: pilih file -> preview (baris
 *  valid vs bermasalah, ditandai jelas) -> "Simpan N baris" -> upsert massal ke server. Bisa dipakai
 *  berkali-kali kapan saja (tidak menghapus data lama, lihat catatan di bulkUpsertItemSellingPricesAction),
 *  berdampingan dengan tombol "+ Tambah SKU" satu-per-satu yang sudah ada. */
export function ImportSkuModal({ onClose }: { onClose: () => void }) {
  const bulkUpsert = useMrpStore((s) => s.bulkUpsertItemSellingPrices);
  const inputRef = useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = useState<string | null>(null);
  const [rows, setRows] = useState<ParsedSkuRow[] | null>(null);
  ...
  async function handleSave() {
    ...
    try {
      const summary = await bulkUpsert(
        validRows.map((r) => ({ kategori: r.kategori, sku: r.sku, itemName: r.itemName, warna: r.warna, lengan: r.lengan!, size: r.size, price: r.price }))
      );
      setResult(summary);
      setRows(null);
      setFileName(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B131B]/45 p-4">
      <div className="flex max-h-[88vh] w-full max-w-[760px] flex-col rounded-lg bg-white shadow-[0_8px_24px_rgba(11,19,27,.2)]">
        <div className="flex items-center justify-between border-b border-border-subtle px-5 py-3.5">
          <span className="font-sans text-[13px] font-semibold text-text-primary">Import Data — SKU (Harga Jual per Item)</span>
          ...
          {result ? (
            ...
            <>
              <div className="flex flex-wrap items-center gap-2">
                ...
                />
                <button onClick={downloadSkuImportTemplate} className="flex-none font-sans text-[11.5px] font-semibold text-action-primary underline">
                  Download Template ↓
                </button>
              </div>
              <div className="mt-1.5 font-sans text-[10.5px] leading-[1.5] text-text-muted">
                Kolom yang dikenali: <span className="font-mono">Kategori/Category</span>, <span className="font-mono">SKU</span> (opsional),{" "}
                <span className="font-mono">Items Name</span> (nama gabungan, mis. &quot;HITAM 24S PDK S&quot;) ATAU <span className="font-mono">Warna</span> +{" "}
                <span className="font-mono">Lengan</span> + <span className="font-mono">Size</span> terpisah, dan{" "}
                <span className="font-mono">Price/Harga Jual</span>. Baris dengan SKU/Warna+Lengan+Size yang SAMA dengan data yang sudah ada akan
                    ...
                    <table className="w-full border-collapse font-sans text-[11px]">
                      <thead className="sticky top-0 bg-[#F2F5F8]">
                        <tr className="text-left uppercase tracking-wider text-text-muted">
                          ...
                          <th className="px-2.5 py-1.5">Lengan</th>
